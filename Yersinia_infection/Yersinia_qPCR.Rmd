...
YR <- read_xlsx("Yersinia_qPCR_data.xlsx", sheet = "R")
...
stat.data <- cbind(md$Infection.Stage, md$Feed,md$Group,YR)
rownames(stat.data) <- md$Sample
stat.data <- melt(stat.data)
colnames(stat.data) <- c("Infection","Feed","Group","Sample", "Variable", "Value")
...
Summary <- stat.data %>%
  ...
  summarise(
    ...
    CQmean = mean(-log2(Value), na.rm = TRUE),
    sd = sd(Value, na.rm = TRUE), 
    CQsd = sd(-log2(Value), na.rm = TRUE)
  )

nz.stat.data <- stat.data
nz.stat.data[is.na(nz.stat.data)] <- 0

stat.test <- nz.stat.data %>%
...
```
```{r}
#pdf("Yersinia_expression.pdf", height = 12, width = 16)
